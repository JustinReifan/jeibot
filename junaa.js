...
const startJuna = async () => {
    ...
    for (let kopel of celled) {
      if (kopel.isGroup == false) {
        if (kopel.status == "offer") {
          let nomer = await juna.sendTextWithMentions(
            kopel.from,
            `*${juna.user.name}* tidak bisa menerima panggilan ${
          ...
          juna.sendContact(
            kopel.from,
            setting.ownerNumber.map((i) => i.split("@")[0]),
            nomer
          );
          await sleep(8000);
  ...
  juna.ev.on("messages.upsert", async (chatUpdate) => {
    try {
      for (let mek of chatUpdate.messages) {
        ...
        if (!juna.public && !mek.key.fromMe && chatUpdate.type === "notify")
          return;
  ...
  juna.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
    juna.sendMessage(
      jid,
      {
        text: text,
  ...
  juna.sendStickerFromUrl = async (from, PATH, quoted, options = {}) => {
    ...
    pathFile = await writeExif(media, {
      packname: options.packname ? options.packname : "Junaa Bot",
  ...
  juna.downloadAndSaveMediaMessage = async (
    message,
    filename,
  ...
  juna.copyNForward = async (
    ...
    await juna.relayMessage(jid, waMessage.message, {
  ...
  juna.sendText = (jid, text, quoted = "", options) =>
    juna.sendMessage(jid, { text: text, ...options }, { quoted, ...options });
