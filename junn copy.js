...
`;
      ...
      case "addsewa":
        ...
        try {
          // Extract and clean invite code from URL
          ...
          // Check if group is already in sewa
          if (_sewa.checkSewaGroup(groupId, sewa))
            return newReply(`Bot sudah disewa oleh grup tersebut!`);

          // Add group to sewa list
          _sewa.addSewaGroup(groupId, args[1], sewa);
          newReply(`Success Add Sewa Group Berwaktu!`);
        } catch (err) {
          console.error("Error in addsewa:", err);
          if (err.data === 400) {
            return newReply("Invalid invite code or invite has expired");
          } else {
            return newReply("Gagal join grup: " + err.message);
          }
        }
        break;
      case "delsewa":
              ...
              await juna.sendMessage(
                m.chat,
                {
                  ...
                  caption:
                    ...
                    `💷 Cek Status Transaksi ketik .ceksuntik\n` +
      ...
      case "facebook": {
        ...
        try {
          ...
          // Filter berdasarkan command (platform)
          const filterKategori = data.filter((s) =>
            s.kategori_rekomendasi.toLowerCase().includes(command)
          );

          if (!filterKategori.length) {
            return newReply("❌ Layanan untuk " + command + " tidak ditemukan");
          }

          // Group by kategori detail
          const grouped = {};
          filterKategori.forEach((s) => {
            if (!grouped[s.kategori_rekomendasi])
              grouped[s.kategori_rekomendasi] = [];
          ...
          let txt = `📦 *Daftar Layanan ${command.toUpperCase()}*\n`;
          txt +=
            "✨ Ketik `.ordersuntik (id layanan) | (jumlah) | (target)` untuk order.\n";
          txt += "━━━━━━━━━━━━━━━━━━━━━━\n\n";
